...
const router_opts = (CFG = DEFAULT_ROUTER_CONFIG) => {
    const rtr = CFG[CFG_RUTR] || null
    const pre = isEmpty(CFG[RTR_PREP]) ? null : CFG[RTR_PREP]
    const pst = isEmpty(CFG[RTR_POST]) ? null : CFG[RTR_POST]
 ...
 * ```
 */
export const __DOM_URL__ROUTE = (CFG = DEFAULT_ROUTER_CONFIG, SET_STATE) => {
