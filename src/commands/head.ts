...
const defalt_cfg = {
  ...
  favicon: document.querySelector("link[rel*='icon']"), //"https://github.com/loganpowell/ac/raw/master/assets/favicon.ico",
}
...
export const INJECT_HEAD: any = registerCMD({
  ...
  [CMD_WORK]: ({
    [URL_DATA]: {
      [DOM_HEAD]: { title, description, img_url, img_height, img_width, favicon, type },
    },
  }: apiURL) =>
    replaceMeta(
      conformToHead({ title, description, img_url, img_height, img_width, favicon, type })
    ),
})
