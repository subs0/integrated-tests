...
const feedCMDtoTarget$fromSource$ = (cmd, target$ = command$) => {
...
export const registerCMDtoStore = (store, stream$ = null) => (command = null) => {
    if (!command)
    ...
    if (src$)
        feedCMDtoTarget$fromSource$(command, stream$);
